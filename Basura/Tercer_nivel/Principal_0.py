...
# from Segundo_Nivel.SegundoNivel import puntaje
W, H = 1500, 900
FPS = 30
...
def realizar_accion_boss(pantalla, accion_enemigo, lados_enemigo):
    ...
    match accion_enemigo:
        case "Quieto":
                animar(pantalla, lados_enemigo["main"], boss_quieto)
        case "Quieto izquierda":
                animar(pantalla, lados_enemigo["main"], boss_quieto_izquierda)
        case "Ataque uno":
                animar(pantalla, lados_enemigo["main"], boss_ataque_uno)
                # mover_personaje(lados_enemigo, velocidad)
        case "Ataque uno izquierda":
                animar(pantalla, lados_enemigo["main"], boss_ataque_uno_izquierda)
                # mover_personaje(lados_enemigo, velocidad * -1)
        case "Ataque dos":
                animar(pantalla, lados_enemigo["main"], boss_ataque_dos)
                # mover_personaje(lados_enemigo, velocidad)
        case "Ataque dos izquierda":
                animar(pantalla, lados_enemigo["main"], boss_ataque_dos_izquierda)
                # mover_personaje(lados_enemigo, velocidad * -1)
        case "Daño":
                animar(pantalla, lados_enemigo["main"], boss_daño)
...
while True:
    ...
    elif contador_daño_a_boss > 5:
        
        if rectangulo_boss.x > rectangulo_personaje.x:
            if rectangulo_boss.colliderect(rectangulo_personaje):
                accion_boss = "Ataque Dos izquierda"
                ataque_enemigo = 1
            else:
                accion_boss = "Quieto izquierda"
        elif rectangulo_boss.x < rectangulo_personaje.x:
            if rectangulo_boss.colliderect(rectangulo_personaje):
                accion_boss = "Ataque Dos"
                ataque_enemigo = 1
            else:
                accion_boss = "Quieto"
        realizar_accion_boss(PANTALLA, accion_boss, lados_boss)
    ...
    elif personaje_flecha.colliderect(rectangulo_boss):
        ataque_personaje = 0
        accion_enemigo = "Daño"
        contador_daño_a_boss += 1
        desaparecer_flecha(personaje_flecha, posicion_actual_x,  posicion_actual_y)
        if contador_daño_a_boss == 10:
            muerte = 1
            puntaje += 1000
            realizar_accion_boss(PANTALLA, accion_boss, lados_boss)
    elif contador == 5 and personaje_flecha.x > 0 and personaje_flecha.x < W:
        ataque_personaje = 0
    ...
    # if contador_daño_a_personaje == 1:   
    PANTALLA.blit(item_vida,pocima_vida)

    if rectangulo_personaje.colliderect(lados_item_vida["main"]):
        desaparecer_moneda(lados_item_vida["main"])
        contador_daño_a_personaje -= 1
